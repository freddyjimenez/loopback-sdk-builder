  ...
  **/
  ...
  function buildModelProperties(model, isInterface) {
    ...
    Object.keys(model.properties).forEach((property) => {
      ...
      if(meta.type.name==="ModelConstructor"||typeof meta.type==="object") return;
    ...
    Object.keys(model.sharedClass.ctor.relations).forEach(relation => {
      ...
      output.push( `  ${relation.charAt(0).toLowerCase()}${relation.slice(1)}${isInterface ? '?' : ''}: ${relationType}${defaultTypeValue};` );
    });
    return output.join('\n');
  }
  /**
  ...
  function buildRelationType(model, relationName) {
    ...
    let finalType = relation.type.match(/(hasOne|belongsTo|embedsOne)/g)
      ? basicType : `${basicType}[]`;
    return finalType;
  }
  /**
